...
function openpgp_crypto_getRandomBytes(length) {
	...
	for (var i = 0; i < length; i++) {
		result += String.fromCharCode(openpgp_crypto_getSecureRandom(0,255));
	}
	return result;
}
...
function openpgp_crypto_getSecureRandom(from, to) {
	...
	while ((buf[0] >> (32 - bits)) > (to-from))
		window.crypto.getRandomValues(buf);
	return from+Math.abs(buf[0] >> (32 - bits));
}
...
function openpgp_crypto_getRandomBigInteger(bits) {
	   ...
	   if (numBytes > 0) {
		   var excessBits = 8*numBytes - bits;
		   randomBits = String.fromCharCode((1 << (8-excessBits)) -1) + randomBits.substring(1);
	   }
